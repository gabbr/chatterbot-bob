...
public class DialogueManager
{
  ...
  public String getNextResponse(String userUtt, String lang)
  {
    if (!(lang.toUpperCase().equals("EN")
            || lang.toUpperCase().equals("DE") || lang.toUpperCase().equals("IT")))
    {
          ...
          String userUtt, String lang)
  {
    ...
    {
      System.err.println("ERROR in getAllPossibleNormalResponses(): getNextNodes_allPossibleNormalPatterns() returned null!");
    }
  ...
  NodeList getAllNormallyReachableNodes()
  {
    ...
    {
      System.err.println("ERROR in getAllNormallyReachableNodes(): Could not find any topic!");
    }
    return nodelist;
  }
  ...
  private Node getNextNode(String userUtt, String lang)
  {
    ...
    if (sdMode == true)
    {
       ...
       * false; C = match; if (logging) { log.debug(
    ...
    if (logging)
    {
      ...
      for (int x = reranked.size() - 1; x >= 0; x--)
      {
        log.debug(sessionID.substring(0, 8)
                + " DEBUG: #"
                + tt.getNodeName(tt.getNodeFromTopicID(Quadruple.get1(reranked.get(x)), sessionID), sessionID));
      }
    }
    // SD mode setting allowed
     ...
     * if (logging) { log.debug(sessionID +
  ...
  public void resetDialogue()
  {
    ...
    continuousNoPatternFoundErrors = 0;
    C = tt.getRootNode();
    System.out.println(sessionID.substring(0, 8)
