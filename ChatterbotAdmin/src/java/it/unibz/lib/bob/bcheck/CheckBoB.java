    ...
    try
    {
      if (dm != null)
      {
        for (int i = 0; i < infile.questions.size(); i++)
        {
          ...
          Vector<Quadruple<String, String, String, String>> vresp = dm.getAllPossibleNormalResponses(infile.questions.get(i), lang.toUpperCase());
                    ...
                    || dm.topicIDsAreEquivalent(Tuple.get2(firstMatch), infile.ids.get(i)))
            {
              ...
              if (trainingMode)
              {
                ...
                trainingCsv_out.append(quan.getTrainingData(
                        infile.questions.get(i), infile.ids.get(i), vresp, lang));
        ...
        System.out.println("BoB's answer correctness: "
        ...
        System.out.println("D - Correct match, many solutions: "
                + countCorrectMultisol_luckyOrder);
        System.out.println("E - wrong match, many solutions, including correct one: "
                + countWrongMultisol_unluckyOrder);
        System.out.println("F - !!! wrong match, many solutions, all wrong: "
                + countWrongMultisol_allBad);
        if (trainingMode
                && (idfEN != null || idfDE != null || idfIT != null))
        {
          System.out.println("****** How the answer reranker WOULD have done here: ");
          System.out.println("D->E - !!! after machine learning reranker: "
                  + reranked_DtoE);
          System.out.println("E->D - after machine learning reranker: "
                  + reranked_EtoD);
